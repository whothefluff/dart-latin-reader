...
import 'package:latin_reader/src/component/library/library.drift.dart';
...
final operations = [
  ...
  (
    ...
    insert: (AppDb db) async {
      final csvData = await rootBundle.loadString(
        '${path}work_content_subdivisions.csv',
      );
      final rows = const CsvToListConverter().convert(csvData);
      await db.batch(
        (b) => b.insertAll(
  ...
  (
    ...
    insert: (AppDb db) async {
      final csvData = await rootBundle.loadString(
        '${path}work_content_supplementary.csv',
      );
      final rows = const CsvToListConverter().convert(csvData);
      await db.batch(
        (b) => b.insertAll(
