...
final operations = [
  (
    ...
    insert: (AppDb db) async {
      final csvData = await rootBundle.loadString(
        '${_path}morphological_details.csv',
      );
        ...
        (b) {
          ...
          return b.insertAll(
              ...
              rows
                  .skip(1)
                  .where(dictRefIsSet)
                  .map((row) => MorphologicalDetailsCompanion(
  ...
  (
    ...
    insert: (AppDb db) async {
      final csvData = await rootBundle.loadString(
        '${_path}morphological_detail_inflections.csv',
      );
      final rows = const CsvToListConverter().convert(csvData);
      await db.batch(
        (b) => b.insertAll(
            db.morphologicalDetailInflections,
            rows.skip(1).map((row) => MorphologicalDetailInflectionsCompanion(
                  form: Value(row[0].toString()),
                  item: Value(row[1] as int),
