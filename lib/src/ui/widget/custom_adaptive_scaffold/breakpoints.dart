...
class Breakpoint {
  ...
  bool isActive(BuildContext context) {
    final host = Theme.of(context).platform;
    final isRightPlatform = platform?.contains(host) ?? true;
    final width = MediaQuery.sizeOf(context).width;
    final height = MediaQuery.sizeOf(context).height;
    final orientation = MediaQuery.orientationOf(context);
    final isPortrait = orientation == Orientation.portrait;
    final lowerBoundWidth = beginWidth ?? double.negativeInfinity;
    final upperBoundWidth = endWidth ?? double.infinity;
    final lowerBoundHeight = beginHeight ?? double.negativeInfinity;
    final upperBoundHeight = endHeight ?? double.infinity;
    final isWidthActive = andUp
        ? width >= lowerBoundWidth
        : width >= lowerBoundWidth && width < upperBoundWidth;
    final isHeightActive = isPortrait || isWidthActive || andUp
  ...
  static Breakpoint? maybeActiveBreakpointFromSlotLayout(BuildContext context) {
    final slotLayout = context.findAncestorWidgetOfExactType<SlotLayout>();
  ...
  static Breakpoint defaultBreakpointOf(BuildContext context) =>
      activeBreakpointIn(context, Breakpoints.all) ?? Breakpoints.standard;

  /// Returns the currently active [Breakpoint].
  static Breakpoint activeBreakpointOf(BuildContext context) =>
      maybeActiveBreakpointFromSlotLayout(context) ?? defaultBreakpointOf(context);
  ...
  static bool isDesktop(BuildContext context) =>
      Breakpoint.desktop.contains(Theme.of(context).platform);

  /// Returns true if the current platform is Mobile.
  static bool isMobile(BuildContext context) =>
      Breakpoint.mobile.contains(Theme.of(context).platform);
  ...
  bool between(Breakpoint lower, Breakpoint upper) => this >= lower && this < upper;
}
