...
typedef NavigationRailDestinationBuilder =
    NavigationRailDestination Function(
      int index,
...
class AdaptiveScaffold extends StatefulWidget {
  ...
  static Builder standardNavigationRail({
    ...
    final actualWidth = extended && width == 72 ? 192.0 : width;
    return Builder(
      ...
      return NavigationBarTheme(
        data: currentNavBarTheme.copyWith(
          iconTheme: WidgetStateProperty.resolveWith(
            (states) =>
                currentNavBarTheme.iconTheme?.resolve(states)?.copyWith(size: iconSize) ??
  ...
  static Builder toMaterialGrid({
    ...
    builder: (context) {
      final currentBreakpoint = Breakpoint.activeBreakpointIn(
        context,
        breakpoints,
      );
      final thisMargin = margin ?? currentBreakpoint?.margin ?? kMaterialCompactMargin;
      final thisColumns = itemColumns ?? currentBreakpoint?.recommendedPanes ?? 1;
  ...
  static AnimatedWidget leftOutIn(Widget child, Animation<double> animation) => SlideTransition(
    position: Tween<Offset>(
  ...
  static AnimatedWidget leftInOut(Widget child, Animation<double> animation) => SlideTransition(
    position: Tween<Offset>(
      begin: Offset.zero,
  ...
  static AnimatedWidget rightOutIn(Widget child, Animation<double> animation) => SlideTransition(
    position: Tween<Offset>(
      begin: const Offset(1, 0),
      end: Offset.zero,
    ).animate(animation),
  ...
  /// Fade in animation.
  static Widget fadeIn(Widget child, Animation<double> animation) => FadeTransition(
    opacity: CurvedAnimation(parent: animation, curve: Curves.easeInCubic),
    child: child,
  );

  /// Fade out animation.
  static Widget fadeOut(Widget child, Animation<double> animation) => FadeTransition(
  ...
  static Widget stayOnScreen(Widget child, Animation<double> animation) => FadeTransition(
...
class _AdaptiveScaffoldState extends State<AdaptiveScaffold> {
          ...
          config: <Breakpoint, SlotLayoutConfig>{
            ...
            widget.largeBreakpoint: SlotLayout.from(
              ...
              builder: (_) => AdaptiveScaffold.standardNavigationRail(
                ...
                padding: widget.navigationRailPadding,
                selectedIndex: widget.selectedIndex,
                destinations: widget.destinations.map(AdaptiveScaffold.toRailDestination).toList(),
                onDestinationSelected: widget.onSelectedIndexChange,
                backgroundColor: navRailTheme.backgroundColor,
                selectedIconTheme: navRailTheme.selectedIconTheme,
                unselectedIconTheme: navRailTheme.unselectedIconTheme,
        ...
        body: SlotLayout(
          config: <Breakpoint, SlotLayoutConfig?>{
            ...
            if (widget.extraLargeBody != null)
              widget.extraLargeBreakpoint: (widget.extraLargeBody != AdaptiveScaffold.emptyBuilder)
                  ? SlotLayout.from(
        ...
        secondaryBody: SlotLayout(
          config: <Breakpoint, SlotLayoutConfig?>{
            ...
            if (widget.mediumLargeSecondaryBody != null)
              widget.mediumLargeBreakpoint:
                  (widget.mediumLargeSecondaryBody != AdaptiveScaffold.emptyBuilder)
                  ? SlotLayout.from(
            ...
            if (widget.extraLargeSecondaryBody != null)
              widget.extraLargeBreakpoint:
                  (widget.extraLargeSecondaryBody != AdaptiveScaffold.emptyBuilder)
                  ? SlotLayout.from(
  ...
  void _onDrawerDestinationSelected(int index) {
    if (widget.useDrawer) {
      ...
      final scaffoldCurrentContext = _scaffoldKey.currentState;
      ...
      children: <Widget>[
        Expanded(
          child: CustomMultiChildLayout(
            ...
            children: children.map<Widget>((child) => LayoutId(id: i += 1, child: child)).toList(),
          ),
        ),
      ],
    );
...
class _BrickLayoutDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    final itemConstraints = BoxConstraints(
      maxWidth:
          fullWidthConstraints.maxWidth / columns - columnSpacing / 2 - itemPadding.horizontal,
    );
    ...
    final columnWidth = (size.width - totalColumnSpacing) / columns;
    final topPadding = itemPadding.resolve(TextDirection.ltr).top;
    final columnUsage = List<double>.generate(columns, (index) => topPadding);
    for (final childSize in childSizes) {
      positionChild(
        childId,
        Offset(
  ...
  bool shouldRelayout(_BrickLayoutDelegate oldDelegate) =>
      itemPadding != oldDelegate.itemPadding || columnSpacing != oldDelegate.columnSpacing;
  //
}
