...
class SlotLayout extends StatefulWidget {
  ...
  static SlotLayoutConfig? pickWidget(
    ...
    final breakpoint = Breakpoint.activeBreakpointIn(
      context,
      config.keys.toList(),
    );
    return breakpoint != null && config.containsKey(breakpoint) ? config[breakpoint] : null;
  }
  ...
  Widget build(context) {
    ...
    return AnimatedSwitcher(
      ...
      transitionBuilder: (child, animation) {
        final configChild = child as SlotLayoutConfig;
      ...
      child: chosenWidget ?? const SlotLayoutConfig.empty(),
    );
  }
...
class SlotLayoutConfig extends StatelessWidget {
  ...
  @override
  Widget build(context) => (builder != null) ? builder!(context) : const SizedBox.shrink();
  //
}
